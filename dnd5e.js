...
Hooks.once("ready", function() {
  ...
  const systemMigrationVersion = "1.6.0";
  const currentVersion = systemMigrationVersion;
  const NEEDS_MIGRATION_VERSION = "1.6.0";
  const COMPATIBLE_MIGRATION_VERSION = 0.80;
  const totalDocuments = game.actors.size + game.scenes.size + game.items.size;
  if ( !currentVersion && totalDocuments === 0 ) return game.settings.set("dnd5e", "systemMigrationVersion", systemMigrationVersion);
