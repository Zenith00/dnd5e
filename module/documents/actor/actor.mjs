...
export default class Actor5e extends Actor {
  ...
  _prepareCharacterData() {
    ...
    // );
    // this.system.attributes.martialChar = data.attributes.martialLevel >= Math.floor(((2 / 3) * data.details.level));
    // this.system.attributes.martialProf = Math.floor((data.attributes.martialLevel + 7) / 4);
  ...
  async _preUpdate(changed, options, user) {
    ...
    if ( isDead && (foundry.utils.getProperty(changed, "system.attributes.hp.value") > 0) ) {
      ...
      // foundry.utils.setProperty(changed, "system.attributes.exhaustion", Math.min(6, this.data.data.attributes.exhaustion + 1));
  ...
  async rollHitDie(denomination, options={}) {
    ...
    const hitDiceHealMultiplier = game.settings.get("dnd5e", "hitDiceMultiplier");
    // Prepare roll data
    const flavor = game.i18n.localize("DND5E.HitDiceRoll");
    const rollConfig = foundry.utils.mergeObject({
      formula: `max(0, ${hitDiceHealMultiplier || 1}${denomination} + @abilities.con.mod)`,
