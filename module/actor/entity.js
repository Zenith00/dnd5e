...
export default class Actor5e extends Actor {
  ...
  prepareDerivedData() {
    ...
    // ActorData.flags.dnd5e.saveDC = data.abilities[data.attributes.spellcasting]?.dc || (10 + data.prof?.flat);
    ...
    for (let [id, abl] of Object.entries(data.abilities)) {
      ...
      abl.save = abl.mod + abl.saveBonus + (this.type === "npc" ? game.settings.get("dnd5e", "globalMonsterBonusSave") : 0);
  ...
  _prepareCharacterData(actorData) {
    ...
    if (!data.resources.fourth) {
      data.resources.fourth = {};
    }
    if (!data.resources.fifth) {
      data.resources.fifth = {};
    }
  ...
  _computeArmorClass(data) {
    ...
    switch (ac.calc) {
      ...
      case "natural":
        ac.base = Number(ac.flat) + (this.type === "npc" ? game.settings.get("dnd5e", "globalMonsterBonusAC") : 0);
        break;
  ...
  async _preUpdate(changed, options, user) {
    ...
    if (isDead && (foundry.utils.getProperty(changed, "data.attributes.hp.value") > 0)) {
      ...
      foundry.utils.setProperty(changed, "data.attributes.exhaustion", Math.min(6, this.data.data.attributes.exhaustion + 1));
  ...
  rollAbilitySave(abilityId, options = {}) {
    ...
    let baseBonuses = this.type === "npc" ? {save: game.settings.get("dnd5e", "globalMonsterBonusSave")} : {};
    // Include a global actor ability save bonus
    const bonuses = getProperty(this.data.data, "bonuses.abilities") || {};
    if (bonuses.save) {
      parts.push("@saveBonus");
      data.saveBonus = Roll.replaceFormulaData(bonuses.save, data);
    } else if (baseBonuses.save) {
      parts.push("@saveBonus");
      data.saveBonus = Roll.replaceFormulaData(baseBonuses.save, data);
    }
  ...
  async rollDeathSave(options = {}) {
    ...
    // Const secret = ;
    // const whisperTargets = secret ? game.users.filter(u => u.isGM).map(u => u.id) : [];
    ...
    if (game.settings.get("dnd5e", "forceSecretDeathSave")) {
      roll.options.rollMode = CONFIG.Dice.rollModes.gmroll;
    }
  ...
  async shortRest({dialog = true, chat = true, autoHD = false, autoHDThreshold = 3} = {}) {
    return this.mediumRest({dialog, chat, newDay: true});
    ...
    // }
    // return this._rest(
    //   chat, newDay, false, false, this.data.data.attributes.hd - hd0, this.data.data.attributes.hp.value - hp0);
  }
  ...
  async _rest(chat, newDay, longRest, mediumRest = false, dhd = 0, dhp = 0) {
    ...
    if (longRest) {
      result.updateData["data.resources.tertiary.value"] = 0;
    }
